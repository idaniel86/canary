...
events.onerror = (error) => {
    console.error("SSE connection error", error, events.readyState);
};

events.onmessage = (event) => {
    console.log("Unnamed SSE event", event.data);
};

events.addEventListener("quality_score", (event) => {
...
function updateSubScore(field, subScore) {
    ...
    const { score, value } = subScore;

    document.getElementById(`value-${field}`).textContent = `${value.toFixed(1)} ${def.unit}`;
